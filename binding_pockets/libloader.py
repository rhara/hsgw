...
def compute_binding_pocket_features(pocket_featurizer, ligand_featurizer, pdb_subdir, pdb_code, threshold=.3):
    ...
    # Extract active site
    active_site_box, active_site_atoms, active_site_coords = (dc.dock.binding_pocket.extract_active_site(protein_file, ligand_file))
    ...
    pocket_features = pocket_featurizer.featurize(protein_file, pockets, pocket_atoms, pocket_coords)
    ...
    for ind, pocket in enumerate(pockets):
        overlap = dc.dock.binding_pocket.compute_overlap(pocket_atoms, active_site_box, pocket)
...
def load_pdbbind_labels(labels_file):
    ...
    contents_df = pd.DataFrame(contents, columns=('PDB code', 'resolution', 'release year', '-logKd/Ki', 'Kd/Ki',
                                                'ignore-this-field', 'reference', 'ligand name'))
    return contents_df

def featurize_pockets(subset='core', trunc_dataset_size=None):
    """Featurizes pdbbind according to provided featurization"""
    print('@libloader.featurize_pdbbind_pockets')
    print('subset', subset)
    tasks = ['active-site']
    ...
    pdbbind_dir = '/tmp/v2015'
    print('pdbbind_dir', pdbbind_dir)
    ...
    for ind, pdb_code in enumerate(ids):
        ...
        all_ids.append(ids)
        if trunc_dataset_size is not None and trunc_dataset_size <= len(all_ids):
            break
    time2 = time.time()
...
def load_pockets(split='index', subset='core'):
    """Load PDBBind datasets. Does not do train/test split"""
    print('@libloader.load_pockets')
    dataset, tasks = featurize_pockets(subset=subset)
